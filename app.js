...
async function scanPriceTag(file) {
    ...
    try {
        ...
        if (result.success && result.data) {
            ...
            // 2. Auto-set Amount
            if (data.total) {
                setInput(data.total); // Use setInput for consistent UI updates
...
function setInput(val) {
    if (elements.localAmount) {
        ...
        // Manual toggle of clear button if function exists or manual check
...
function initOCRListeners() {
    console.log("Initializing OCR Listeners (Overwritten)...");
